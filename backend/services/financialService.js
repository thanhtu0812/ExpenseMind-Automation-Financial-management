...
function getDateRange(timePeriod, refDateStr) {
    ...
    switch (timePeriod) {
        case 'day':
            ...
            break;
        case 'week':
            startDate = new Date(refDate.setDate(refDate.getDate() - refDate.getDay()));
            startDate.setHours(0, 0, 0, 0);
...
async function getSavingsSuggestion(userId) {
    ...
    }).populate('category_id', 'name').sort({ date: -1 });
...
async function getMonthlyReport(userId, type, date) {
    ...
    const transactions = await Transaction.find(query).populate('category_id', 'name');

    return transactions;
}

async function createTransaction(userId, type, amount, categoryName, description, transactionDate) {
    const category = await Category.findOne({ name: categoryName, $or: [{ user_id: new mongoose.Types.ObjectId(userId) }, { is_default: true }] });
    if (!category) {

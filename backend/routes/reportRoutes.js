...
async function createReportPDF(userId, reportType, dateRange) {
  ...
  const filePath = path.join(
    __dirname,
    `../reports/${userId}_${reportType}.pdf`
  );
  ...
  doc
    .fontSize(20)
    .text(`Financial Report (${reportType})`, { align: "center" });
  doc.moveDown();
  ...
  transactions.forEach((t) => {
    doc.text(
      `- ${t.date.toLocaleDateString()}: ${t.category_id.name} - ${t.type} - ${
        t.amount
      } VND`
    );
  });
...
router.post("/schedule", protect, async (req, res) => {
  ...
  try {
    if (sendMode === "now") {
      ...
      return res
        .status(201)
        .json({ message: "Report scheduled successfully!" });
    }
  } catch (error) {

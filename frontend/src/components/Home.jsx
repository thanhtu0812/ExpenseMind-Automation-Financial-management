...
const Home = () => {
  ...
  // Fetch cả expense và income stats
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [overviewRes, expenseCatRes, incomeCatRes] = await Promise.all([
          ...
          axios.get(
            `${API_URL}/api/transactions/stats/by-category?type=expense`,
            config
          ),
          axios.get(
            `${API_URL}/api/transactions/stats/by-category?type=income`,
            config
          ),
        ]);
    ...
    >
      ...
      <div className="chart-section">
          ...
          {activeChart === 0 ? (
            <>
              <div className="chart-card fade-in">
                <h3 className="chart-title expense">
                  Expense{" "}
                ...
                <p className="total-text chart-total">
                  ...
                  <span className="total-amount expense">
                    {" "}
                    {overview.expense?.toLocaleString() || 0}đ{" "}
                  </span>
                </p>
              </div>

              <div className="chart-card fade-in">
                <h3 className="chart-title income">
                  Income{" "}
                  <span className="chart-date">
                    {new Date().toLocaleDateString("en-GB", {
                      month: "numeric",
                ...
                <p className="total-text chart-total">
                  ...
                  <span className="total-amount income">
                    {" "}
                    {overview.income?.toLocaleString() || 0}đ{" "}
                  </span>
                </p>
              </div>
            </>
            ...
            <div className="chart-card-wide fade-in">
              <div className="chart-left">
                <h3 className="chart-title correlation">
                  {" "}
                ...
                >
                  ...
                  <RechartsLegend />
                  <Bar dataKey="Income" fill="#7A5AF8" />{" "}
                  <Bar dataKey="Expense" fill="#F28482" />
                </BarChart>
              </div>
              <div className="chart-info">
                ...
                <p>
                  ...
                  {overview.balance?.toLocaleString() || 0}đ{" "}
                </p>
              </div>
            </div>
          )}
        ...
        <div className="chart-dots">
          <span
            className={`dot ${activeChart === 0 ? "active" : ""}`}
            onClick={() => setActiveChart(0)}
          ></span>
          <span
            className={`dot ${activeChart === 1 ? "active" : ""}`}
      ...
      <div className="button-section">
        <div className="feature-card" onClick={() => navigate("/add")}>
          {" "}
          <img src={expenseIcon} alt="Expense" /> <p>Expense Management</p>{" "}
        </div>
        <div className="feature-card" onClick={() => navigate("/report")}>
          {" "}
          <img src={reportIcon} alt="Report" /> <p>Report</p>{" "}
        </div>
        <div className="feature-card" onClick={() => navigate("/chatbot")}>
          {" "}
          <img src={chatbotIcon} alt="AI ExpenseMind" />{" "}
          <p>Talk with AI ExpenseMind</p>{" "}
        </div>
      </div>
